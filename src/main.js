...
class Game {
  constructor() {
    ...
    this.settings = new Settings({ tuning: TUNING, drivers: DRIVERS, storage: window.localStorage });
    // effective config: rebuilt in place from the settings so renderer, HUD and world share one object
    this.tuning = deepClone(TUNING); this.profiles = deepClone(DRIVERS); this.graphics = this.settings.build('graphics');
    ...
    this.settingsMenu = new SettingsMenu(this.settings, {
      onClose: () => { this.settingsMenu.hide(); if (this.applySettings()) this.settingsBack(); },
      onCustom: () => { this.settingsMenu.hide(); this.startCustom(); },
    });
    this.sfx = new Sfx();
  ...
  applySettings() {
    const tuning = this.settings.build('tuning'), profiles = this.settings.build('drivers');
    try {
      validateAll(profiles, [], tuning, BODIES);
    } catch (e) {
      this.menus.show({
        title: 'Bad settings', lines: [e.message],
        buttons: [
          { label: 'Settings', onClick: () => this.showSettings(this.settingsBack) },
          { label: 'Reset all', onClick: () => { this.settings.reset(); this.settings.save(); if (this.applySettings()) this.settingsBack(); } },
        ],
      });
      return false;
    }
  ...
  showTitle() {
    this.custom = null; this.paused = false; this.world = null;
    ...
    this.menus.show({
      ...
      buttons: [
        ...
        { label: '1st‑Squirrel', onClick: () => { this.setFirstPerson(true); this.start(startAt, {}); } },
  ...
  setFirstPerson(on) {
    ...
    this.settings.set('graphics.firstPerson', on); this.settings.save();
  ...
  start(index, carry) {
    ...
    try {
      ...
      this.menus.show({
        title: 'Bad custom level', lines: [e.message],
        buttons: [
          { label: 'Settings', onClick: () => this.showSettings(() => this.showTitle()) },
  ...
  render(alpha, frameTime) {
    ...
    if (st !== this.lastPlayerState) {
      if (st === 'hopping') { if (w.player.dur === tn.dashTime) this.sfx.dash(); else this.sfx.hop(); }
