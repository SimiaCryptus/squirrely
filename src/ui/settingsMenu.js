...
export class SettingsMenu {
  ...
  build() {
    ...
    intro.innerHTML =
      'Everything is tunable: physics, every driver, rules, scoring, graphics — and a custom road. ' +
    ...
    for (const g of s.schema) {
      ...
      if (g.colorHex !== undefined)
        sum.style.color = `#${g.colorHex.toString(16).padStart(6, '0')}`;
      d.appendChild(sum);
  ...
  field(f) {
    ...
    input.addEventListener('change', () => {
      if (f.type === 'boolean') s.set(f.path, input.checked);
        ...
        if (Number.isFinite(v)) s.set(f.path, v);
      }
      s.save();
      this.sync(f, input);
    });
  ...
  sync(f, input) {
    ...
    if (f.type === 'boolean') input.checked = !!v;
    else input.value = String(v);
    input.classList.toggle('changed', this.settings.isOverridden(f.path));
  }

  refresh() {
    for (const { f, input } of this.inputs.values()) this.sync(f, input);
  }
  show() {
    this.refresh();
