...
export class Sfx {
  ...
  noise(dur, { gain = 0.3, pan = 0, cutoff = 900 } = {}) {
    if (!this.ctx || this.voices > 20) return;
    const c = this.ctx,
      n = Math.max(1, Math.floor(c.sampleRate * dur));
    ...
    s.connect(f);
    f.connect(this._out(0, dur, gain, pan));
    s.start();
    this.voices++;
    s.onended = () => {
      this.voices--;
  ...
  hop() {
    this.tone(480, 0.07, { type: 'triangle', gain: 0.07, f1: 720 });
  }
  dash() {
    this.tone(300, 0.18, { type: 'triangle', gain: 0.09, f1: 900 });
  }
  ...
  bind(events, world) {
    ...
    events.on('horn', ({ vehicle: v }) => {
      if (!near(v)) return;
      const red = v.driverId === 'red',
        green = v.driverId === 'green';
    ...
    events.on('bump', ({ a }) => {
      if (near(a)) this.noise(0.12, { gain: 0.15, pan: pan(a), cutoff: 600 });
    });
    events.on('crash', ({ a, severity }) => {
      if (!near(a)) return;
      this.noise(severity === 'wreck' ? 0.7 : 0.45, {
        gain: severity === 'wreck' ? 0.5 : 0.35,
        pan: pan(a),
      });
    ...
    events.on('taunt', () => {
      for (let i = 0; i < 4; i++)
        this.tone(1800, 0.04, { type: 'square', gain: 0.06, f1: 2600, at: i * 0.06 });
    });
    events.on('player:nudge', () =>
      this.tone(200, 0.15, { type: 'triangle', gain: 0.12, f1: 120 })
    );
    events.on('item:pickup', ({ kind }) => {
      if (kind === 'smoke')
        this.tone(700, 0.12, { type: 'triangle', gain: 0.09, f1: 420 }); // heavy "thunk"
      else this.tone(900, 0.08, { type: 'triangle', gain: 0.09, f1: 1500 });
    });
    events.on('smoke:lit', () => {
      this.noise(0.7, { gain: 0.25, cutoff: 1600 });
      this.tone(1400, 0.5, { type: 'sine', gain: 0.05, f1: 200 });
    });
    events.on('player:death', ({ cause }) => {
      if (cause === 'timer') {
        this.tone(300, 0.4, { type: 'square', gain: 0.1, f1: 80 });
        return;
      }
    ...
    events.on('hollow', () =>
      [523, 659, 784].forEach((f, i) =>
        this.tone(f, 0.15, { type: 'triangle', gain: 0.1, at: i * 0.09 })
      )
    );
    events.on('level:complete', () =>
      [523, 659, 784, 1046].forEach((f, i) =>
        this.tone(f, 0.25, { type: 'triangle', gain: 0.12, at: 0.4 + i * 0.12 })
      )
    );
    events.on('game:over', () =>
      [400, 300, 200].forEach((f, i) =>
        this.tone(f, 0.3, { type: 'square', gain: 0.08, at: i * 0.25 })
      )
    );
  }
}
