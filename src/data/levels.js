...
const L = (dir, speedLimit, density, flags = {}) => ({
  type: 'lane',
  dir,
  speedLimit,
  density,
...
export const LEVELS = [
  ...
  },
  {
    ...
    mix: { white: 0.3, purple: 0.2, yellow: 0.15, green: 0.13, blue: 0.12, red: 0.1 },
  },
  {
    id: 'l07',
    name: 'Gridlock',
  ...
  {
    id: 'l11',
    name: 'Rush Hour Redux',
    timeLimit: 50,
    platoonChance: 0.35,
    modifiers: { rushHour: true },
    rows: [
      V(),
      L(1, 16, 26, A),
      L(1, 18, 28),
  ...
  },
  {
    ...
    timeLimit: 45,
    platoonChance: 0.35,
    modifiers: { night: true, rain: true },
    rows: [
      V(),
      S(),
      L(1, 16, 26, A),
...
export function endlessLevel(n) {
  ...
  for (let i = 0; i < half; i++) {
    rows.push(
      L(1, 14 + i * 2, Math.round((22 + i * 2) * k), i === 0 ? A : i === half - 1 ? C : {})
    );
  }
  rows.push(M());
  for (let i = lanes - half - 1; i >= 0; i--) {
    rows.push(
      L(-1, 14 + i * 2, Math.round((22 + i * 2) * k), i === 0 ? A : i === lanes - half - 1 ? C : {})
    );
  }
  rows.push(G());
  return {
    id: `endless-${n}`,
    name: `Endless · Crossing ${n}`,
    timeLimit: 45,
    platoonChance: 0.3,
    endless: true,
    rows,
    mix: { white: 0.28, purple: 0.2, yellow: 0.16, green: 0.14, blue: 0.12, red: 0.1 },
  };
}
/**
...
export function customLevel(c, n = 1) {
  ...
  for (let i = each - 1; i >= 0; i--)
    rows.push(L(-1, c.baseSpeed + i * c.speedStep, dens(i), flags(i)));
  if (c.shoulders) rows.push(S());
  rows.push(G());
  const modifiers = {};
  ...
  return {
    id: `custom-${n}`,
    name: `Custom · Crossing ${n}`,
    timeLimit: c.timeLimit,
    platoonChance: c.platoonChance,
    custom: true,
    rows,
    mix,
    modifiers,
