...
export class World {
  constructor({ level, drivers, tuning, seed = 1, events, carry = {} }) {
    ...
    this.road = new Road(
      level.rows,
      tuning,
      (mods.rushHour ? 0.8 : 1) * (mods.winter ? 0.85 : 1),
      mods.winter ? tuning.winterTraction : 1
    );
    this.vehicles = [];
    this.pool = [];
    this._sorted = [];
  ...
  step(dt, intent = EMPTY_INTENT) {
    ...
    this.time += dt;
    this.stepCount++;
    if (warm) this.events.clear();
    else this.events.flush(); // 10
  }

  drive(v, dt) {
    const drv = this.drivers[v.driverId],
      tn = this.tuning;
    ...
    eff = this.frustrate(v, eff, P, dt);
    v.eff = eff; // a'. temper behind a slow leader (multispeed traffic, §7.6)
    let aLong = P.leader
    ...
    intent.targetLane = null;
    intent.aLong = aLong;
    intent.emergency = false;
    intent.ignoreLeader = false;
    intent.horn = false;
    intent.noMobil = false;
    intent.slam = false;
    ...
    if (
      ...
      !pl.behind &&
      pl.gap > 0 &&
      (pl.inPath || pl.predictedInPath) &&
      v.state === 'cruise' &&
      !intent.targetLane
    ) {
      const lane = v.lane,
        maxOff = lane.width * 0.5 - v.width * 0.5 - 0.45;
      ...
      v.lateralOffset = side * maxOff;
      v.flinchTimer = 0.8;
      if (
        !intent.slam &&
        Math.abs(pl.predictedZ - zNew) > halfPath + 0.2 &&
        Math.abs(pl.z - zNew) > halfPath + 0.2
      ) {
    ...
    if (tl && tl !== v.lane && tl.dir === v.dir) {
      ...
      if (intent.emergency) this.beginLaneChange(v, tl, true);
        ...
        if (eff.indicatorLead <= 0.05) this.beginLaneChange(v, tl, false);
          ...
          v.lcPending = tl;
          v.lcPendingTimer = eff.indicatorLead;
    ...
    if (v.noiseTimer <= 0) {
      v.noiseTimer = 0.25;
      v.brakeNoiseVal = eff.brakeNoise ? eff.brakeNoise * v.rng.norm() : 0;
    }
  ...
  frustrate(v, eff, P, dt) {
    const L = P.leader;
    const stuck =
      L && L.veh.state !== 'wrecked' && L.s < 30 && L.veh.v < 0.8 * v.v0 && v.v < 0.9 * v.v0;
    v.frustration =
      stuck && eff.impatience > 0
        ? Math.min(1, v.frustration + eff.impatience * dt)
        : Math.max(0, v.frustration - 0.2 * dt);
    const f = v.frustration;
    if (f < 0.01) return eff;
    if (eff !== v.effScratch) {
      Object.assign(v.effScratch, eff);
      eff = v.effScratch;
    } // params() drivers already work in the scratch
    eff.T *= 1 - 0.45 * f;
    eff.s0 *= 1 - 0.35 * f;
    eff.lcThreshold *= 1 - 0.8 * f;
    eff.p -= 0.4 * f;
    eff.bSafe += 2 * f;
    if (f > 0.7 && L && L.s < 12 && this.time % 1.5 < 0.25)
  ...
  honk(v) {
    ...
    if (v.rng() < v.eff.hornChance) {
      v.hornTimer = 1.5;
      this.events.emit('horn', { vehicle: v });
    }
  }
  ...
  spawnVehicle({ lane, driverId, bodyType, x, v, v0, v0Jit = 1 }) {
    ...
    veh.reset({
      ...
      v0,
      v0Jit,
      rng: makeRng(splitmix32(this.seed ^ Math.imul(id, 0x9e3779b9))),
      time: this.time,
    });
  ...
  cull() {
    ...
    for (const v of this.vehicles) {
      ...
      if (dead) {
        this.pool.push(v);
        this.events.emit('despawn', { vehicle: v });
  ...
  onCollision(a, b, dv, severity, x, z) {
    ...
    const p = this.player,
      dx = x - p.x,
      dz = z - p.z;
    if (!this.warm && this.status === 'playing' && p.alive && dx * dx + dz * dz < 225)
      this.scoring.chaos();
  ...
  session(dt) {
    ...
    if (p.state === 'squashed') {
      p.deathTimer -= dt;
      if (p.deathTimer <= 0) this.respawnAfterDeath();
      return;
    }
    if (p.state === 'goal') {
      p.goalTimer -= dt;
      if (p.goalTimer <= 0) this.respawnAfterGoal();
      return;
    }
    this.timer -= dt;
    if (this.timer <= 0) {
      this.timer = 0;
      this.killPlayer(null, 'timer');
      return;
    }
    if (p.justLanded) {
      p.justLanded = false;
      ...
      if (row) {
        if (row.index > p.furthestRow) {
          p.furthestRow = row.index;
          this.scoring.advance();
        }
  ...
  tryHollow() {
    ...
    p.reachGoal();
    this.events.emit('hollow', {
      index: this.hollows.indexOf(best),
      remaining: this.hollows.length - this.hollowsFilled,
    });
  ...
  nearMiss() {
    ...
    for (const v of this.vehicles) {
      ...
      if (!v.nearMissed && d < tn.nearMissRadius && v.speed > tn.nearMissSpeed) {
        ...
        if (this.slowMos < tn.slowMoCap) {
          this.slowMos++;
          this.slowMoTimer = tn.slowMoTime;
        }
  ...
  respawnAfterGoal() {
    const p = this.player,
      mouth = p.mouth.slice(); // smoke bombs ride along to the next crossing
    p.reset();
    p.mouth.push(...mouth);
    this.timer = this.timeLimit;
    this.slowMos = 0;
  ...
  hash() {
    ...
    h = hashFloat(h, this.player.x);
    h = hashFloat(h, this.player.z);
    return h;
  }
}
