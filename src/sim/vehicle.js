...
export class Vehicle {
  constructor(tuning) {
    ...
    this.percept = {
      ...
      follower: null,
      player: this.ring[0],
      _leader: { veh: null, s: 0 },
      _follower: { veh: null, s: 0 },
      _cur: playerSnapshot(),
    };
    this.intent = {
      targetLane: null,
  ...
  reset(o) {
    ...
    this.speedFactor = b.speedFactor ?? 1;
    this.v0Jit = o.v0Jit ?? 1; // personal speed bias, kept across lanes (§7.6)
    ...
    this.omega = 0;
    this.wreckTimer = 0;
    this.signals.brake = false;
    this.signals.indicator = 0;
    this.signals.hazard = false;
    this.signals.headlightFlash = 0;
    this.rng = o.rng;
    this.spawnTime = o.time;
    this.ringHead = 0;
    for (const r of this.ring) r.visible = false;
    this.percept._cur.visible = false;
    this.percept.player = this.ring[0];
    this.percept.leader = null;
    this.percept.follower = null;
  ...
  integrate(dt, road) {
    ...
    if (eff.steerNoise > 0 || tr < 1) {
      ...
      if (tr < 1 && this.aCmd < -0.6 * eff.bMax)
        this.wander += (1 - tr) * 1.2 * this.rng.norm() * Math.sqrt(dt);
    ...
    const vzMax =
      Math.min(emergency ? eff.vzMaxEmergency : eff.vzMax, Math.max(0.35, this.v * 0.3)) *
      this.agility *
      (0.5 + 0.5 * tr);
    ...
    if (this.state === 'changing' || this.state === 'evading') {
      this.lcTimer += dt;
      const settled =
        Math.abs(this.z - this.lane.zCenter) < 0.35 * this.lane.width && Math.abs(dz) < 0.6;
  ...
  wreck(vx, vz, omega, lifetime) {
    ...
    this.wz = vz;
    this.omega = omega;
    this.wreckTimer = lifetime;
    this.v = 0;
    this.aCmd = 0;
    this.lcPending = null;
    this.signals.hazard = true;
    this.signals.indicator = 0;
    this.signals.brake = false;
  }
}
