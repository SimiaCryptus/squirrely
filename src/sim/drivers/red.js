...
function aim(v, pl, target, intent) {
  if (target && target !== v.lane) {
    ...
    if (nb) {
      intent.targetLane = nb;
      intent.emergency = true;
    } // no indicator, no MOBIL safety, re-targets mid-swerve
  }
...
export function tick(v, P, intent, ctx) {
  ...
  switch (v.mode) {
    ...
    case 'LOCKED':
      v.lockTimer += dt;
      intent.noMobil = true;
      if (!pl.visible || pl.behind || v.lockTimer > ex.lockDuration) {
        v.mode = 'RECOVER';
        break;
      }
    ...
    case 'COMMIT':
      intent.noMobil = true;
      if (!pl.visible || pl.behind || pl.ttc > 1.5) {
        v.mode = 'RECOVER';
        break;
      }
