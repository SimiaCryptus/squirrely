...
export function tick(v, P, intent, ctx) {
  ...
  if (v.triggerTimer <= 0) {
    ...
    for (const o of w.vehicles) {
      ...
      if (dx < 12 && (o.state === 'changing' || o.state === 'evading')) add += 0.1;
    ...
    for (const s of w.items.smokes) {
      ...
      if (dx > -2 && dx < 25 && Math.abs(s.z - v.z) < 10) {
        add += 0.12;
        break;
      }
    }
    if (add > 0) {
      ...
      if (
        ...
        v.stabCooldown <= 0 &&
        v.panic > 0.25 &&
        rng() < Math.min(1, add * 2 + v.panic * 0.3)
      ) {
        ...
        if (v.flinchTimer <= 0 && rng() < 0.6) {
          // in-lane flinch away from the scare
  ...
  if (v.stabTimer > 0) {
    v.stabTimer -= dt;
    intent.aLong = Math.min(intent.aLong, v.stabAccel);
    intent.slam = true;
  ...
  if (squirrel && pl.ttc < 2.5) {
    intent.aLong = Math.min(intent.aLong, -eff.bMax);
    intent.slam = true;
  }
  ...
  if (v.rollTimer <= 0) {
    ...
    if (
      v.panic > 0.5 &&
      v.state === 'cruise' &&
      v.lane.neighbors.length &&
      rng() < ex.swerveChance * v.panic
    ) {
      const side = awaySide(v, pl, rng);
      let lane = null;
      for (const nb of v.lane.neighbors)
        if (Math.sign(nb.zCenter - v.lane.zCenter) === side) lane = nb;
      if (!lane) lane = rng.pick(v.lane.neighbors);
      intent.targetLane = lane;
      intent.emergency = true;
      intent.noMobil = true; // ignores MOBIL safety
