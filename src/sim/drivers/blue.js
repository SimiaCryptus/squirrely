...
function safestAdjacent(v, pz, world) {
  ...
  for (const nb of v.lane.neighbors) {
    const L = per.leaderIn(nb, v),
      F = per.followerIn(nb, v);
    const lg = L ? per.gap(v, L) : Infinity,
      fg = F ? per.gap(v, F) : Infinity;
...
export function tick(v, P, intent, ctx) {
  ...
  if (aStop < -eff.bComf * 0.9 && v.state === 'cruise') {
    // braking alone would be uncomfortable: swerve
    ...
    if (lane) {
      ...
      v.lateralOffset =
        Math.sign(lane.zCenter - v.lane.zCenter) * (ex.overcompensate - 1) * lane.width;
      v.flinchTimer = 1.2;
      if (!v.swerving) {
