...
export function tick(v, P, intent, ctx) {
  ...
  if (lid !== v.lastLeaderId) {
    // someone new in front: were we cut off?
    if (L && (L.veh.state === 'changing' || L.veh.state === 'evading')) {
      const closing = v.v - L.veh.v;
      if (closing > 0 && L.s / closing < 1.5) {
        v.rage = Math.min(1, v.rage + 0.25);
        intent.horn = true;
      }
    }
    v.lastLeaderId = lid;
  ...
  if (v.rollTimer <= 0) {
    ...
    if (
      ...
      for (const nb of v.lane.neighbors) {
        const nl = per.leaderIn(nb, v),
          nf = per.followerIn(nb, v);
        const lg = nl ? per.gap(v, nl) : Infinity,
          fg = nf ? per.gap(v, nf) : Infinity;
