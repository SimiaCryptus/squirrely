...
const SHAPE = {
  moto: {
    body: [0.9, 0.0, 0.35, 0.45],
    cab: [0.4, -0.15, 0.35, 1.0, 0.55],
    glass: [0.06, 0.38, 0.5, 0.85, 0.6],
  },
  hatch: { body: [1.0, 0.0, 0.55], cab: [0.5, -0.02, 0.55, 1.0], glass: [0.54, -0.02, 0.6, 0.9] },
  sedan: { body: [1.0, 0.0, 0.5], cab: [0.5, -0.05, 0.5, 1.0], glass: [0.54, -0.05, 0.56, 0.88] },
  suv: { body: [1.0, 0.0, 0.5], cab: [0.62, -0.02, 0.5, 1.0], glass: [0.66, -0.02, 0.56, 0.88] },
  pickup: { body: [1.0, 0.0, 0.48], cab: [0.34, 0.12, 0.48, 1.0], glass: [0.38, 0.12, 0.54, 0.9] },
  van: { body: [1.0, 0.0, 0.55], cab: [0.85, -0.04, 0.55, 1.0], glass: [0.88, -0.04, 0.6, 0.88] },
  box: { body: [0.66, -0.17, 1.0], cab: [0.28, 0.35, 0.0, 0.62], glass: [0.3, 0.35, 0.3, 0.55] },
  bus: { body: [1.0, 0.0, 1.0], cab: null, glass: [0.96, 0.0, 0.48, 0.78] },
};
...
const _e = new THREE.Euler(0, 0, 0, 'YXZ'),
  ...
  _c2 = new THREE.Color();
const COL = {
  brake: new THREE.Color(0xff2a1a),
  tail: new THREE.Color(0x4a0d0d),
  tailNight: new THREE.Color(0x9a1c1c),
  amber: new THREE.Color(0xffa826),
  off: new THREE.Color(0x222222),
  headDay: new THREE.Color(0xb8b09a),
  headNight: new THREE.Color(0xfff3c4),
  flash: new THREE.Color(0xffffff),
};
...
export class VehicleView {
  constructor(scene) {
    ...
    for (const type in BODY_TYPES) {
      ...
      const glass = makeInstanced(
        part(b, sh.glass, sh.glass[4] ?? (sh.cab ? 0.92 : 1.06)),
        glassMat,
        CAP
      );
    ...
    this.lights = makeInstanced(
      new THREE.BoxGeometry(0.14, 0.22, 0.36),
  ...
  update(world, alpha, time) {
    for (const t in this.bodies) {
      const e = this.bodies[t];
      e.body.count = 0;
      e.glass.count = 0;
      if (e.cab) e.cab.count = 0;
    }
    let nl = 0,
      nw = 0;
    ...
    for (const v of world.vehicles) {
      ...
      const x = lerp(v.prevX, v.x, alpha),
        z = lerp(v.prevZ, v.z, alpha);
      const h = lerpAngle(v.prevHeading, v.heading, alpha);
      const wrecked = v.state === 'wrecked';
      const moto = v.bodyType === 'moto';
      const rollK = moto ? 0.18 : 0.05,
        rollMax = moto ? 0.4 : 0.1; // bikes lean into their swerves
      ...
      const hl = b.length * 0.5 - 0.08,
        hw = moto ? 0 : b.width * 0.5 - 0.22;
      ...
      const wl = b.length * 0.32,
        ww = moto ? 0 : b.width * 0.5 - 0.1;
