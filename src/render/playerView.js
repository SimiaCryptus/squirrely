...
export class PlayerView {
  constructor(scene) {
    ...
    for (const m of [body, chest, head, earL, earR, eyeL, eyeR, this.tail, this.held]) {
      m.castShadow = true;
      this.mesh.add(m);
    }
  ...
  update(p, alpha, time) {
    ...
    if (p.state === 'goal') {
      const k = clamp(p.goalTimer / 0.8, 0, 1);
      sy *= k;
      sxz *= k;
    }
