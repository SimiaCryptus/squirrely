...
export class RoadView {
  constructor(scene, road, tuning, modifiers = {}) {
    const winter = !!modifiers.winter,
      colors = winter ? WINTER_COLORS : ROW_COLORS;
    this.group = new THREE.Group();
    scene.add(this.group);

    const ground = new THREE.Mesh(
      new THREE.PlaneGeometry(600, 300),
      new THREE.MeshLambertMaterial({ color: winter ? 0xdde3e8 : 0x2f5d2a })
    );
    ground.rotation.x = -Math.PI / 2;
    ground.position.set(0, -0.05, road.zMin / 2);
    ground.receiveShadow = true;
    this.group.add(ground);

    for (const row of road.rows) {
      const m = new THREE.Mesh(
        new THREE.PlaneGeometry(260, row.depth),
        new THREE.MeshLambertMaterial({ color: colors[row.type] })
      );
      m.rotation.x = -Math.PI / 2;
      m.position.set(0, 0, row.zCenter);
    ...
    for (let i = 0; i < road.rows.length - 1; i++) {
      ...
      if (a.lane && b.lane) {
        if (a.lane.dir === b.lane.dir) dashes.push(z);
          ...
          solids.push({ z: z + 0.12, c: 0xf2c724 });
          solids.push({ z: z - 0.12, c: 0xf2c724 });
        }
    ...
    const inst = new THREE.InstancedMesh(
      ...
      new THREE.MeshBasicMaterial({ color: 0xe8e8e0 }),
      Math.max(1, dashes.length * nDash)
    );
    const dummy = new THREE.Object3D();
    let k = 0;
    for (const z of dashes)
    ...
    for (const s of solids) {
      const m = new THREE.Mesh(
        new THREE.BoxGeometry(260, 0.02, 0.15),
    ...
    if (goal) {
      for (const x of tuning.hollowXs) {
        const trunk = new THREE.Mesh(
          new THREE.CylinderGeometry(0.5, 0.7, 2.6, 8),
          new THREE.MeshLambertMaterial({ color: 0x6b4a2b })
        );
        trunk.position.set(x, 1.3, goal.zCenter - 0.6);
        trunk.castShadow = true;
        const crown = new THREE.Mesh(
          new THREE.SphereGeometry(1.6, 10, 8),
          new THREE.MeshLambertMaterial({ color: winter ? 0x8fb2a0 : 0x3f8f3f })
        );
        crown.position.set(x, 3.3, goal.zCenter - 0.6);
