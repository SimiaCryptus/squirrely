...
export class ItemView {
  ...
  update(world, time) {
    ...
    for (const it of items.list) {
      ...
      _m.compose(_p, _q, _s);
      if (isBomb) {
        this.bombs.setMatrixAt(nb, _m);
        this.fuses.setMatrixAt(nb, _m);
        nb++;
      } else {
        this.acorns.setMatrixAt(n, _m);
        this.caps.setMatrixAt(n, _m);
        n++;
      }
    }
    this.acorns.count = n;
    this.acorns.instanceMatrix.needsUpdate = true;
    this.caps.count = n;
    this.caps.instanceMatrix.needsUpdate = true;
    this.bombs.count = nb;
    this.bombs.instanceMatrix.needsUpdate = true;
    this.fuses.count = nb;
    this.fuses.instanceMatrix.needsUpdate = true;
