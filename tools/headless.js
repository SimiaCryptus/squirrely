...
const levelId = args.level ?? 'l04',
  seeds = Number(args.seeds ?? 20),
  seconds = Number(args.seconds ?? 60);
const level = levelId.startsWith('endless-')
  ? endlessLevel(Number(levelId.split('-')[1]))
...
const acc = {
  ...
  vehSamples: 0,
  samples: 0,
  duty: level.rows.filter((r) => r.type === 'lane').map(() => 0),
};
const t0 = Date.now();
...
for (let seed = 1; seed <= seeds; seed++) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    acc.samples++;
    acc.vehSamples += world.vehicles.length;
    world.road.lanes.forEach((lane, li) => {
      let open = true;
      for (const v of world.vehicles) {
        if (v.lane !== lane && v.lcFrom !== lane) continue;
        if (Math.abs(v.x) < v.length * 0.5 + 1) {
          open = false;
          break;
        }
        const ahead = lane.dir * (0 - v.x);
        if (ahead > 0 && ahead / Math.max(v.speed, 0.1) < 1.2) {
          open = false;
          break;
  ...
  acc.crashes += world.stats.crashes;
  acc.bumps += world.stats.bumps;
  acc.swerves += world.stats.swerves;
  acc.locks += world.stats.locks;
}
...
if (args.json) console.log(JSON.stringify(report));
  ...
  console.log(
    `Level ${report.level} — ${seeds} seeds × ${seconds} s (${report.wallSeconds} s wall)`
  );
  console.log(
    `  crashes/min ${report.crashesPerMin}   bumps/min ${report.bumpsPerMin}   swerves/min ${report.swervesPerMin}   locks/min ${report.locksPerMin}`
  );
