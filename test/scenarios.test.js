...
function duel(driverId, seed = 7) {
  ...
  const world = new World({
    level,
    drivers: createDriverRegistry(DRIVERS),
    tuning: TUNING,
    seed,
    events,
  });
  ...
  p.x = 0;
  p.z = lane.zCenter;
  p.row = lane.row.index;
  p.prevX = p.x;
  p.prevZ = p.z;
  const car = world.spawnVehicle({
    lane,
    driverId,
    bodyType: 'sedan',
    x: -40 * lane.dir,
...
test('Red swerves toward a squirrel standing in the neighbouring lane', () => {
  ...
  assert.ok(
    Math.abs(car.z - lane1.zCenter) < 1.5,
    `should be nearly on top of the squirrel's lane, z=${car.z}`
  );
...
test('White brakes to a stop for the player and never changes lane', () => {
  ...
  let laneChanges = 0;
  events.on('lanechange', ({ vehicle }) => {
    if (vehicle === car) laneChanges++;
  });
...
test('property: no NaN, vehicles stay inside the carriageway (5 seeds × 20 s, Rush Hour)', () => {
  ...
  for (let seed = 1; seed <= 5; seed++) {
    ...
    const zMax = world.road.laneZMax + 0.5,
      zMin = world.road.laneZMin - 0.5;
    for (let i = 0; i < 20 * 120; i++) {
      world.step(dt, EMPTY_INTENT);
      for (const v of world.vehicles) {
        assert.ok(
          Number.isFinite(v.x) && Number.isFinite(v.z) && Number.isFinite(v.v),
          'finite state'
        );
